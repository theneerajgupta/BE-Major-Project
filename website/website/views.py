...
def predictor(url) :
    ...
    output = {
        ...
        'userid': user['screen_name'],
        'tweet': tweet['text'],
        'sentiment': f"SENTIMENT : {'POSITIVE' if prediction == 1 else 'NEGATIVE'}",
...
def output(request) :
    url = str(request.GET['weburl'])
    dictionary = predictor(url)
    return render(request, 'output.html', dictionary)
